...
contract SonarMeta is Storage, Events, ReentrancyGuard, Context, Config, Ownable {
    ...
    function applyForAirdrop() external nonReentrant {
        ...
        emit Fund(_msgSender(), AIRDROP_AMOUNT);
    }
    ...
    function mintERC721(address _to) external nonReentrant returns(uint256 _tokenId){
        governance.requireController(_msgSender());
        require(_to!= address(0), "ti0");
        _tokenId = ERC721ModelCollection.mint(_to);
    ...
    function mintERC998(address _to) external nonReentrant returns(uint256 _tokenId){
        governance.requireController(_msgSender());
        require(_to!= address(0), "ti0");
        _tokenId = ModifiedERC998SceneCollection.mint(_to);

        emit MintSceneFor(_to, _tokenId);
    }

    function mintERC998WithBatchTokens(address _to, uint256[] calldata _childTokenIds) external nonReentrant returns(uint256 _tokenId) {
        governance.requireController(_msgSender());
        require(_to!= address(0), "ti0");
        _tokenId = ModifiedERC998SceneCollection.mintFromBatchTokens(_to, _childTokenIds);
